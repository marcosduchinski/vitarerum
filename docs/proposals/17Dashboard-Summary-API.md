...
> None of the summary endpoints described below exist in the backend. This
> document is a design proposal, not a contract of current behaviour. See
> [SPEC-021](../specs/021-resumos-de-painel/spec.md) for the specified
> requirements and the declared implementation gap. The `/p/dashboard` page
> itself exists in the UI, backed by the reused endpoints listed at the end of
> this document.
...
- A `dashboard.view` permission resource and a per-resource access-control
  layer (`/p/admin/access-control`, migration `0015_access_control`) have been
  described elsewhere as if partly implemented — **none of it exists in the
  current codebase** (the real `0015` migration is
