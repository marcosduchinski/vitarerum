...
### `POST /collection-use-projects/{project_id}/start`
...
**Side effect — the object access log is seeded from the project's objects.** As work begins, the project's `ObjectAccessLog` (`OAL-XXXXXXXX`) is created and one `ObjectLogEntry` is registered for **each `CollectionUseObject`** of the project — these are the objects that will be handled. Each entry links back to its source via `collectionUseObjectId`, sets `numberOfObjects` to `1` (the default — adjust later via `PATCH .../log-entries/{entry_id}`), and records the caller as `addedBy`. The seeded entries are then visible at `GET /collection-use-projects/{project_id}/log-entries`. Seeding only happens when the project has objects **and** no access log exists yet: a project with no objects leaves the log lazily uncreated, and if entries were already added (e.g. by staff before start) the existing log is left untouched.
...
### `POST /collection-use-projects/{project_id}/cancel`
...
`observations` is optional; `numberOfObjects` must be ≥ 1. `collectionUseObjectId` is **required** — it links this access to the `CollectionUseObject` it fulfils (the carrier of the object's inventory snapshot) and must be one of this project's objects (otherwise `422`).
...
`addedBy` is a full permission object, not a bare UUID. The accessed object's inventory snapshot is not duplicated on the entry — resolve it through `collectionUseObjectId` against the project's `objects`.
...
### `GET /collection-use-projects/{project_id}/log-entries`
...
**Query parameters**
```
added_by : UUID     (optional) filter by permissionId
page    : Integer  (default 0)
size    : Integer  (default 20)
```
...
### `GET /collection-use-projects/{project_id}/object-access-log`
...
### `GET /collection-use-projects/{project_id}/log-entries/{entry_id}/attachments/{file_reference}`
...
project_id     : UUID   (required)
entry_id       : UUID   (required)
file_reference : String (required) the attachment's fileReference
```
...
`testimonial` is optional; `numberOfObjects` must be ≥ 1; `occurrenceDate` is when the occurrence happened (client-supplied), `location` and `detailedDescription` are required. `collectionUseObjectId` is **required** — it links this occurrence to the `CollectionUseObject` it concerns (the carrier of the object's inventory snapshot) and must be one of this project's objects (else `422`).
...
### `GET /collection-use-projects/{project_id}/occurrence-entries`
...
### `GET /collection-use-projects/{project_id}/object-occurrence-log`
...
### `POST /collection-use-projects/{project_id}/occurrence-entries/{entry_id}/attachments`
...
### `GET /collection-use-projects/{project_id}/occurrence-entries/{entry_id}/attachments/{file_reference}`
...
### `POST /collection-use-projects/{project_id}/publication-entries`
...
### `PATCH /collection-use-projects/{project_id}/publication-entries/{entry_id}`
...
### `GET /collection-use-projects/{project_id}/publication-log`
...
### `GET /collection-use-projects/{project_id}/publication-entries/{entry_id}/attachments/{file_reference}`
...
**Downstream of the project (staff-only)** — once a project's data is captured, staff can export an `IN_SITU_VISIT` project to a CIDOC-CRM mapping snapshot via `POST /collection-use-projects/{project_id}/export-in-situ-visit-record` (see `05Project phase — staff actions.md` and `08InSituVisit-CIDOC-CRM.md`), which can then be projected to CIDOC-CRM JSON-LD and turned into a narrative (`09KG-RAG-Narrative.md`).
