...
retention decision; see GAP-010 in Section 7.
...
GAP-008 (missing active contexts in the architecture/contributor catalogs) was
resolved by the existing catalog updates on 2026-09-05. Its identifier is
reserved; the remaining enforcement and retention gaps keep GAP-009 and
GAP-010 so external references do not change meaning.

1. **GAP-001** — The aggregate domain-purity contract omits `app.identity.domain`,
...
2. **GAP-002** — The aggregate application-purity contract omits
   `app.reference_numbers.application`. Its current source is framework-free,
   but that fact is not protected by the contract.
3. **GAP-003** — The Notifications layer contract accidentally includes
...
4. **GAP-004** — `public_submission` has explicitly ignored imports into Use of Collections
...
5. **GAP-005** — The contract named `Shared kernel imports stdlib only` does not prove that
...
6. **GAP-006** — There is no catch-all JSON exception handler. Unexpected exceptions and
...
7. **GAP-007** — The shared Angular `ApiError` discards the backend's machine-readable
...
8. **GAP-009** — The context-map update rule is documented but is not itself checked by
   `lint-imports`; diagram checks validate source/render consistency, not
   semantic agreement with Python imports. The context catalog is likewise
   maintained by hand: `docs/architecture/module-boundaries.md` and
   `vitarerum-api/AGENTS.md` listed neither `external_publications` nor
   `reference_numbers` until 2026-09-05, and nothing fails when a new context
   is added without a catalog entry.
9. **GAP-010** — `app/ai/museum_question_triage` contains ORM metadata for a removed
   ...
   The package is now catalogued as an inactive scaffold, but no specification
   owns the retained data. Either drop the tables with a migration or record
   the retention decision and the owning specification.
...
`test/use_of_collections/test_golden_contracts.py::test_golden_proposal_detail_response_shape`,
`test/use_of_collections/test_golden_contracts.py::test_golden_paginated_proposals_envelope_shape`,
