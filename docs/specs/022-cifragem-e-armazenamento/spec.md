...
| --- | --- |
| Field text round trip, confidentiality, and nonce uniqueness | `test/shared/test_field_encryption.py::test_text_round_trip_returns_original_value`, `test/shared/test_field_encryption.py::test_ciphertext_does_not_contain_plaintext`, `test/shared/test_field_encryption.py::test_same_value_encrypts_with_distinct_nonce` |
| Field corruption and AAD mismatch | `test/shared/test_field_encryption.py::test_aad_mismatch_raises_corrupted_field`, `test/shared/test_field_encryption.py::test_missing_prefix_raises_corrupted_field`, `test/shared/test_field_encryption.py::test_unknown_version_raises_corrupted_field`, `test/shared/test_field_encryption.py::test_invalid_base64_raises_corrupted_field`, `test/shared/test_field_encryption.py::test_tampered_ciphertext_raises_corrupted_field` |
| Null preservation, keyed normalization, and JSON | `test/shared/test_field_encryption.py::test_none_remains_none`, `test/shared/test_field_encryption.py::test_email_hash_is_normalized_and_stable`, `test/shared/test_field_encryption.py::test_hash_changes_with_aad`, `test/shared/test_field_encryption.py::test_json_round_trip_returns_original_value`, `test/shared/test_field_encryption.py::test_json_decode_failure_raises_corrupted_field` |
| File round trip, confidentiality, nonce, empty content, and deletion | `test/shared/test_file_encryption.py::test_round_trip_returns_original_content`, `test/shared/test_file_encryption.py::test_saved_blob_does_not_contain_plaintext`, `test/shared/test_file_encryption.py::test_same_content_saves_with_distinct_nonce`, `test/shared/test_file_encryption.py::test_empty_file_round_trips`, `test/shared/test_file_encryption.py::test_delete_delegates_to_inner_storage` |
| File tamper/key/reference/header failures | `test/shared/test_file_encryption.py::test_ciphertext_tampering_raises_corrupted_file`, `test/shared/test_file_encryption.py::test_wrong_key_raises_corrupted_file`, `test/shared/test_file_encryption.py::test_aad_binds_blob_to_file_reference`, `test/shared/test_file_encryption.py::test_blob_without_version_header_raises_corrupted_file`, `test/shared/test_file_encryption.py::test_truncated_versioned_blob_raises_corrupted_file` |
| Local confinement and atomic replacement | `test/shared/test_file_storage.py::test_round_trip_into_subfolder`, `test/shared/test_file_storage.py::test_read_rejects_path_traversal`, `test/shared/test_file_storage.py::test_save_is_atomic_and_leaves_no_temp_files`, `test/shared/test_file_storage.py::test_delete_removes_file_and_is_idempotent` |
| Builder and context file-storage wiring | `test/shared/test_file_storage.py::test_build_file_storage_returns_plain_storage_without_key`, `test/shared/test_file_storage.py::test_build_file_storage_wraps_storage_when_key_is_configured`, `test/shared/test_file_storage.py::test_file_storage_dependencies_use_configured_encryption_key` |
| Runtime key validation | `test/test_config.py::test_non_local_settings_require_file_encryption_key`, `test/test_config.py::test_non_local_settings_require_db_field_encryption_key`, `test/test_config.py::test_file_encryption_key_must_be_valid_base64_in_local`, `test/test_config.py::test_db_field_encryption_key_must_decode_to_32_bytes_in_local` |
| Phase-one repository field wiring | `test/test_database_field_encryption_phase1.py::test_context_encryptor_uses_configured_db_field_key`, `test/test_database_field_encryption_phase1.py::test_museum_questions_wiring_encrypts_through_context_dependencies`, `test/test_database_field_encryption_phase1.py::test_public_submission_repository_stores_selected_fields_encrypted`, `test/test_database_field_encryption_phase1.py::test_museum_question_repository_filters_by_hash_and_encrypts`, `test/test_database_field_encryption_phase1.py::test_amendment_token_repository_stores_requester_email_encrypted` |
| Route-level encrypted file round trip and generic error | `test/use_of_collections/test_api.py::test_log_entry_attachment_uses_configured_encrypted_storage` |
...
| --- | --- | --- | --- |
| GAP-001 | Critical | There is no key identifier, key ring, rotation reader, or rewrite workflow; replacing a key makes every existing value unreadable. | Define envelope/key-version metadata, load current plus previous keys from a secret manager, rotate by authenticated read/re-encrypt, verify counts, and retire old keys only after tested backup/rollback. |
| GAP-002 | Critical | Migration `0054` neither backfills the required digest nor encrypts existing plaintext, and its downgrade does not decrypt ciphertext. | Replace the upgrade assumption with an explicit expand/backfill/verify/contract migration or formally prohibit in-place upgrades and enforce an empty-database precondition. |
| GAP-003 | High | File encryption has no migration path between plaintext, encrypted, or rotated-key blobs. | Add format/key metadata and an idempotent, resumable file rewrite tool with inventory, integrity verification, checkpoints, and rollback/backup instructions. |
| GAP-004 | High | Filename-bearing plaintext `file_reference` values defeat the confidentiality gained by encrypting display filename columns. | Generate opaque references from IDs/extensions only; migrate existing blobs by decrypting/re-encrypting because the reference is AAD, and keep display names solely in protected metadata where required. |
| GAP-005 | High | Sensitive-field coverage is selective and undocumented as a data-classification decision; substantial requester, conversation, research, and AI metadata remains plaintext. | Create an owned data inventory with classification, purpose, retention, search needs, protection decision, AAD/key domain, and compensating controls for every sensitive column. |
| GAP-006 | High | AAD naming is inconsistent and the architecture document claims one physical-name grammar that code does not enforce. | Introduce a reviewed registry of stable versioned AAD constants, test uniqueness/coverage against encrypted mappings, and update `encryption-contracts.md` without deriving labels implicitly from mutable ORM names. |
| GAP-007 | High | Deployment already provisions/preserves both encryption keys in Google Secret Manager, grants the runtime service account access, and injects the `latest` secret versions through `deploy/provision.sh`. The application still has no ciphertext key identifier, startup inventory/key compatibility check, or tested recovery/rotation workflow. | Retain the existing secret-manager integration; add explicit key-version binding and compatibility verification, then exercise restore/rotation before replacing either key. Secret versioning alone does not re-encrypt stored data. |
| GAP-008 | Medium | Enabling/disabling file encryption in local/development can silently change interpretation of an existing data directory. | Persist storage-format metadata or require a new/verified-empty `DATA_DIR`; refuse startup when configuration and on-disk inventory disagree. |
| GAP-009 | Medium | Corruption logs include AAD/file references, and references may contain original filenames. | Log opaque record/reference hashes plus controlled diagnostic context; redact filename-bearing paths and add security metrics/alerts. |
| GAP-010 | Medium | Local path confinement is vulnerable to filesystem races by an actor able to alter symlinks beneath `DATA_DIR`. | Use descriptor-relative, no-follow operations or a storage root not writable by untrusted local actors; add adversarial path tests. |
| GAP-011 | Medium | Atomic replacement is not crash durability, and no backup/restore integrity procedure is specified. | Add file/directory `fsync` where durability is required and document/test encrypted backup restoration with key availability. |
| GAP-012 | Medium | Whole-file encryption and JSON sealing duplicate complete plaintext/ciphertext buffers. | Enforce size limits consistently, measure peak memory/event-loop latency, and adopt an authenticated streaming/container format if larger objects are required. |
| GAP-013 | Medium | HMAC normalization is hard-coded to `strip().lower()` for every lookup domain. | Define per-field canonicalization (for example email/domain or inventory rules), version it with the digest namespace, and test Unicode/case semantics. |
| GAP-014 | Low | Error responses use `500` but offer no stable incident correlation identifier. | Attach a request/incident ID to logs and safe responses without exposing AAD or key details. |
| GAP-015 | Low | `run_with_unique_retry` was previously included as an encryption requirement although it is an unrelated persistence helper. | Keep it documented with the workflows that use it (for example SPEC-010/SPEC-019) rather than expanding this cryptographic boundary. |
