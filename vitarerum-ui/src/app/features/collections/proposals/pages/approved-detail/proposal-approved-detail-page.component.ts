...
@Component({
  selector: 'app-proposal-approved-detail-page',
  standalone: true,
  ...
  templateUrl: './proposal-approved-detail-page.component.html',
  styleUrl: './proposal-approved-detail-page.component.scss',
})
export class ProposalApprovedDetailPageComponent {
  ...
  protected readonly project = computed(() => this.proposal()?.collectionUseProject ?? null);
  ...
  // The approval entry carries who approved it and when — surfaced on the project card.
  protected readonly approvalEvent = computed<ProposalEvent | null>(
    () => this.events().find((event) => event.type === 'APPROVED') ?? null,
  );
