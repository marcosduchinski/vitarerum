...
One system instruction per narrative style; the user prompt carries the validated
CIDOC-CRM graph as the only allowed source of facts.
"""
...
def build_system_prompt(narrative_type: NarrativeType) -> str:
    ...
    return (
        ...
        "structured graph data into a fluid narrative.\n\n"
        "CRITICAL CONSTRAINTS:\n"
        "1. You must ONLY use facts declared in the provided context. Do "
        "not invent events, people, dates or outcomes beyond the graph.\n"
        f"2. Adopt the absolute tone, style and structure of the requested "
        f"narrative type: {narrative_type.value}.\n"
        "3. Output only the narrative text, with no preamble or metadata."
    )


def build_user_prompt(graph: dict[str, Any], target_language: str) -> str:
    return (
        f"[CONTEXT DATA (CIDOC-CRM Validated Graph)]\n"
        f"{json.dumps(graph, ensure_ascii=False)}\n\n"
